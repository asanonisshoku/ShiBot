...
public class MongoHandler {
    ...
    public MongoHandler(MongoDatabase inputDatabase, String collectionCredentials, String miscCredentials, String vtuberCredentials) {
        database = inputDatabase;
        collection = database.getCollection(collectionCredentials);
        misc = database.getCollection(miscCredentials);
        vtubers = database.getCollection(vtuberCredentials);
    }
    ...
    public static BotUser getUser(String userID){
        ...
        if (document == null){
            ...
            //Checking for non-assigned values in the database and keeping default one's if not found, This is fixed when the user re-saves into the database and is for new values
    ...
    /**
     * Searches the user database for users in current server and returns at most the top 5 Users, used in displaying leaderboard stats.
     * @param serverInfo Current Server
     * @return Array of top 5 Users ordered by server currency
     */
    ...
    /**
     * General pull from the database that creates a new entry if the required ID is not found
     * @param userID Full Name of Vtuber
    ...
    /**
     * Pulls a random Gladiator from the database
     * @return Random VtuberGladiator
     */
